...
The Auth feature provides the complete authentication system for MatchLog. It covers user sign-in via Google OAuth and Email/Password, new account registration, email verification, session persistence, and router-level access control. The feature follows Clean Architecture (feature-first) with a Firebase Auth backend for Phase 1–3. An onboarding carousel is shown on first launch before the login screen. Authenticated users are routed to the diary; unauthenticated users are redirected to login. Email/Password users who have not yet verified their email may access the core diary but see a persistent banner until verification is complete.

---

## Glossary

- **AppUser**: The domain entity representing an authenticated user, containing id, email, displayName, photoUrl, emailVerified, tier, and createdAt.
- **AuthRepository**: The abstract domain interface for all authentication operations.
- **FirebaseAuthSource**: The Firebase-backed data source that implements all auth operations against Firebase Auth.
- **AuthRepositoryImpl**: The data-layer implementation that wires FirebaseAuthSource with the local UserProfiles Drift cache.
- **UserProfiles**: The existing Drift table that caches user profile data locally for offline access.
- **AppRouter**: The GoRouter instance that controls all navigation and redirect guards.
- **OnboardingScreen**: A 3-slide carousel shown only on the user's first app launch.
...
- **EmailVerificationBanner**: A persistent in-app banner shown to Email/Password users whose email is not yet verified.
- **UserTier**: Enum with values `free`, `pro`, `crew` representing the user's subscription level.
- **AuthState**: The Riverpod-managed state representing the current authentication status (unauthenticated, authenticated, loading, error).
...
2. WHEN the Google OAuth flow completes successfully, THE AuthRepositoryImpl SHALL create or update the corresponding UserProfiles row in the local Drift database with the returned user data.
...
5. IF the Google Sign-In flow fails due to a network or Firebase error, THEN THE LoginScreen SHALL display a human-readable error message.
6. WHEN a Google-authenticated user's profile is written to UserProfiles, THE AuthRepositoryImpl SHALL set `emailVerified` to `true`.
...
2. WHEN Email/Password sign-in succeeds, THE AuthRepositoryImpl SHALL update the UserProfiles row in the local Drift database with the latest user data.
...
### Requirement 3: Email/Password Registration

**User Story:** As a new user, I want to create an account with my email and password, so that I can start using MatchLog.

#### Acceptance Criteria

1. WHEN the user submits the registration form on RegisterScreen with a valid email and a password of at least 8 characters, THE FirebaseAuthSource SHALL call `createUserWithEmailAndPassword` on Firebase Auth.
...
3. WHEN account creation succeeds, THE AuthRepositoryImpl SHALL insert a new UserProfiles row in the local Drift database with `emailVerified` set to `false` and `tier` set to `UserTier.free`.
...
1. WHEN an Email/Password user is authenticated and `emailVerified` is `false`, THE LoginScreen SHALL display a persistent EmailVerificationBanner with a "Resend email" action.
...
5. WHEN the UserProfiles row is updated to `emailVerified == true`, THE EmailVerificationBanner SHALL be dismissed without requiring a manual app restart.
6. THE AppUser entity SHALL expose `emailVerified` as a `bool` field sourced from the local UserProfiles cache.
7. WHEN a Google-authenticated user signs in, THE AuthRepositoryImpl SHALL always write `emailVerified = true` to the UserProfiles row, regardless of the Firebase `emailVerified` field value.
...
3. WHEN sign-out completes, THE AuthRepositoryImpl SHALL clear the current user's data from the in-memory auth state.
4. IF sign-out fails due to a network error, THEN THE System SHALL still clear the local auth state and redirect to `/login`, treating the session as terminated.

---

### Requirement 6: Auth State Persistence
...
3. WHEN the app starts and no persisted session exists, THE AuthRepositoryImpl SHALL emit a null (unauthenticated) state to the auth state stream.
4. WHEN the auth state stream emits a non-null user, THE AuthRepositoryImpl SHALL read the corresponding UserProfiles row from Drift and merge it with the Firebase user data to construct the AppUser.
...
### Requirement 7: Router Access Control
...
2. WHEN an authenticated user with `emailVerified == true` navigates to `/login` or `/register`, THE AppRouter SHALL redirect the user to `/diary`.
3. WHEN an authenticated user with `emailVerified == false` navigates to any protected route, THE AppRouter SHALL allow navigation to `/diary` and other core diary routes.
4. WHEN an authenticated user with `emailVerified == false` navigates to a social-gated route (feed, groups, group detail, join group), THE AppRouter SHALL redirect the user to `/diary`.
5. THE AppRouter redirect guard SHALL be driven by the Riverpod auth state stream so that route changes are reactive to sign-in and sign-out events.
...
1. WHEN the app is launched for the first time (no prior session and no onboarding-complete flag in shared preferences), THE AppRouter SHALL route the user to `/onboarding` before `/login`.
2. THE OnboardingScreen SHALL display exactly 3 slides with distinct content describing core MatchLog features.
...
### Requirement 9: Offline Profile Access
...
### Requirement 10: Domain Layer Contracts

**User Story:** As a developer, I want well-defined domain interfaces and entities, so that the auth feature can be tested in isolation and swapped to a different backend in Phase 4.

#### Acceptance Criteria

1. THE AuthRepository interface SHALL declare the following methods: `signInWithGoogle()`, `signInWithEmail(email, password)`, `signUpWithEmail(email, password)`, `signOut()`, `getCurrentUser()`, `sendEmailVerification()`, `checkEmailVerified()`, and `authStateChanges`.
2. THE AppUser entity SHALL be a Freezed value object with fields: `id` (String), `email` (String), `displayName` (String), `photoUrl` (String?), `emailVerified` (bool), `tier` (UserTier), and `createdAt` (DateTime).
3. THE AuthRepository interface SHALL use `Either<Failure, T>` or equivalent result types so that callers can handle errors without catching exceptions.
4. THE Domain layer SHALL have zero dependencies on Flutter framework packages, Firebase packages, or Drift — only pure Dart.
...
1. THE test suite SHALL include unit tests for each use case class: `SignInWithGoogle`, `SignInWithEmail`, `SignUpWithEmail`, `SignOut`, `GetCurrentUser`, `SendEmailVerification`, and `CheckEmailVerified`.
2. THE test suite SHALL include unit tests for `AuthRepositoryImpl` covering the online sign-in path, the offline cache-read path, and the email verification update path.
